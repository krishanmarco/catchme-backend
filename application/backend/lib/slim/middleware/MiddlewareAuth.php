...
class MiddlewareAuth {
    ...
    public function __invoke($request, $response, $next) {
        ...
        if (!$authenticate->authenticate())
            return $response->withStatus(401, "Invalid authentication token");
...
class MiddlewareUserAuth {
    ...
    public function __invoke($request, $response, $next) {
        ...
        // Authenticate this user
        $authenticate = new MobileUserAuthHelper($authToken);
        ...
        $controller = new ControllerAccessorAuth($authenticate->getVerifiedUser());
        $this->container = SlimAttrGet::putAuthenticatedController($this->container, $controller);
