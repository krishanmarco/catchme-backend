...
/**
 * This class defines the structure of the 'system_temp_var' table.
 *
 *
 *
...
class SystemTempVarTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = '.Map.SystemTempVarTableMap';
     ...
     */
    const TABLE_NAME = 'system_temp_var';

    /**
     * The related Propel class for this table
     */
    const OM_CLASS = '\\SystemTempVar';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'SystemTempVar';
     ...
     */
    const COL_ID = 'system_temp_var.id';

    /**
     * the column name for the type field
     */
    const COL_TYPE = 'system_temp_var.type';

    /**
     * the column name for the data field
     */
    const COL_DATA = 'system_temp_var.data';

    /**
     * the column name for the expiry_ts field
     */
    const COL_EXPIRY_TS = 'system_temp_var.expiry_ts';
    ...
    public function initialize()
    {
        ...
        $this->setClassName('\\SystemTempVar');
        $this->setPackage('');
        $this->setUseIdGenerator(true);
        // columns
        $this->addPrimaryKey('id', 'Id', 'INTEGER', true, null, null);
        $this->addColumn('type', 'Type', 'TINYINT', true, null, null);
        $this->addColumn('data', 'Data', 'OBJECT', true, null, null);
        $this->addColumn('expiry_ts', 'ExpiryTs', 'INTEGER', true, 10, 0);
    } // initialize()
    ...
    public static function populateObject($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        ...
        if (null !== ($obj = SystemTempVarTableMap::getInstanceFromPool($key))) {
            ...
            $col = $offset + SystemTempVarTableMap::NUM_HYDRATE_COLUMNS;
        } else {
            $cls = SystemTempVarTableMap::OM_CLASS;
            /** @var SystemTempVar $obj */
            $obj = new $cls();
            $col = $obj->hydrate($row, $offset, false, $indexType);
            SystemTempVarTableMap::addInstanceToPool($obj, $key);
        }
    ...
    public static function populateObjects(DataFetcherInterface $dataFetcher)
    {
        ...
        while ($row = $dataFetcher->fetch()) {
            $key = SystemTempVarTableMap::getPrimaryKeyHashFromRow($row, 0, $dataFetcher->getIndexType());
            if (null !== ($obj = SystemTempVarTableMap::getInstanceFromPool($key))) {
                // We no longer rehydrate the object, since this can cause data loss.
                // See http://www.propelorm.org/ticket/509
    ...
    public static function addSelectColumns(Criteria $criteria, $alias = null)
    {
        if (null === $alias) {
            ...
            $criteria->addSelectColumn(SystemTempVarTableMap::COL_DATA);
            $criteria->addSelectColumn(SystemTempVarTableMap::COL_EXPIRY_TS);
        } else {
            $criteria->addSelectColumn($alias . '.id');
            $criteria->addSelectColumn($alias . '.type');
            $criteria->addSelectColumn($alias . '.data');
            $criteria->addSelectColumn($alias . '.expiry_ts');
        }
    }
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \SystemTempVar) { // it's a model object
...
} // SystemTempVarTableMap
// This is the static code needed to register the TableMap for this table with the main Propel class.
//
SystemTempVarTableMap::buildTableMap();
